...
void check_malloced_chunk(void *mem, size_t s) {           
    if (mem != 0) {                        
        ...
        size_t sz = p->head & ~CHUNK_ALIGN_MASK;
        CHECK_INUSE_CHUNK(p);
...
{    
    ...
    size_t size = (bytes <= MIN_REQUEST) ? MIN_CHUNK_SIZE : PAD_SIZE(bytes); 

    assert(!(size >> (SIZE_T_BITSIZE-1)));  // check for negative numbers
